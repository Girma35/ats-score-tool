      ...
      <div className="flex items-start gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1.5">
            <span className={`text-[11px] font-black uppercase tracking-wide px-2 py-0.5 rounded-md ${style.labelStyle}`}>
              {style.label}
            </span>
          </div>
          <h3 className={`text-sm font-black ${style.titleColor} leading-snug`}>
            {item.title}
          </h3>
          <p className="text-sm text-gray-900 font-bold mt-1.5 leading-relaxed">{item.description}</p>
        </div>
      </div>
    </div>
  );
